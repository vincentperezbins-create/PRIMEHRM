  ...
  <div class="row">
    ...
    <div class="col-lg-8 mb-20"><div class="card-box pd-20">
      ...
      <div class="table-responsive"><table class="table table-bordered"><thead><tr><th>Title</th><th>Inclusive Date</th><th>Daily Forms</th><th>Participants</th><th>Evaluations</th><th>Action</th></tr></thead><tbody>
        <?php foreach ($trainings as $training): ?><tr><td><a href="ld_training_view.php?code=<?= urlencode($training['trainingmatrixUCODE']) ?>"><?= htmlspecialchars($training['trainingmatrixTITLE'], ENT_QUOTES, 'UTF-8') ?></a></td><td><?= htmlspecialchars($training['trainingmatrixINCLUSIVEDATE'] ?: '-', ENT_QUOTES, 'UTF-8') ?></td><td><?= (int) $training['day_count'] ?></td><td><?= (int) $training['participant_count'] ?></td><td><?= (int) $training['evaluation_count'] ?></td><td><div class="d-flex flex-wrap" style="gap:6px;"><a class="btn btn-sm btn-outline-info" href="ld_training_view.php?code=<?= urlencode($training['trainingmatrixUCODE']) ?>">View Details</a><form method="POST" class="mb-0"><input type="hidden" name="token" value="<?= htmlspecialchars($token, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="generate_certificates"><input type="hidden" name="trainingmatrix_ucode" value="<?= htmlspecialchars($training['trainingmatrixUCODE'], ENT_QUOTES, 'UTF-8') ?>"><button class="btn btn-sm btn-primary" type="submit">Generate Certificates</button></form><form method="POST" class="mb-0"><input type="hidden" name="token" value="<?= htmlspecialchars($token, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="generate_certificate_pdfs"><input type="hidden" name="trainingmatrix_ucode" value="<?= htmlspecialchars($training['trainingmatrixUCODE'], ENT_QUOTES, 'UTF-8') ?>"><button class="btn btn-sm btn-outline-primary" type="submit">Save PDFs Bulk</button></form><a class="btn btn-sm btn-outline-secondary" href="ld_certificates_bulk_print.php?code=<?= urlencode($training['trainingmatrixUCODE']) ?>" target="_blank">Print Bulk</a></div></td></tr><?php endforeach; ?>
